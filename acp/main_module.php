...
class main_module
{
    ...
    public function main($id, $mode)
    {
        ...
        if (isset($config['mundophpbb_supporttriage_priority_enable']))
        {
            $sql = 'SELECT priority_key, COUNT(topic_id) AS total
                FROM ' . $status_table . '
        ...
        if (isset($config['mundophpbb_supporttriage_kb_enable']))
        {
            $sql = 'SELECT COUNT(source_topic_id) AS total
                FROM ' . $table_prefix . 'supporttriage_kb_links';
            $result = $db->sql_query($sql);
        ...
        if ($logs_supported)
        {
            $sql = 'SELECT COUNT(log_id) AS total
                FROM ' . $this->logs_table($table_prefix);
            $result = $db->sql_query($sql);
            $row = $db->sql_fetchrow($result);
            $db->sql_freeresult($result);
    ...
    protected function notices_table($table_prefix)
    {
        return $table_prefix . 'supporttriage_notices';
    }
...
'
            . '    FROM ' . TOPICS_TABLE . ' t
'
            . '    LEFT JOIN ' . $this->status_table($table_prefix) . ' st
'
...
'
    ...
    protected function count_active_notices($db, $table_prefix)
    {
        ...
        $sql = 'SELECT COUNT(notice_id) AS total
'
            . '    FROM ' . $this->notices_table($table_prefix) . '
'
            . '    WHERE is_active = 1';
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
...
'
        ...
        $sql = 'SELECT forum_id, COUNT(topic_id) AS total
'
            . '    FROM ' . TOPICS_TABLE . '
'
...
'
        ...
        $sql = 'SELECT forum_id, COUNT(post_id) AS total
'
            . '    FROM ' . POSTS_TABLE . '
'
...
'
        ...
        foreach ($forum_ids as $forum_id)
        {
            ...
            $topic_total = isset($topic_counts[$forum_id]) ? (int) $topic_counts[$forum_id] : 0;
            $post_total = isset($post_counts[$forum_id]) ? (int) $post_counts[$forum_id] : 0;
            $requires_review = ($topic_total > 0 || $post_total > 0);

            $rows[] = [
                'FORUM_ID' => $forum_id,
                'FORUM_NAME' => isset($forums[$forum_id]) ? $forums[$forum_id] : $user->lang('ACP_SUPPORTTRIAGE_APPROVAL_FORUM_UNKNOWN', $forum_id),
    ...
    protected function build_approval_summary(array $rows)
    {
        ...
        foreach ($rows as $row)
        {
            $summary['unapproved_topics'] += isset($row['UNAPPROVED_TOPICS']) ? (int) $row['UNAPPROVED_TOPICS'] : 0;
            $summary['unapproved_posts'] += isset($row['UNAPPROVED_POSTS']) ? (int) $row['UNAPPROVED_POSTS'] : 0;
        }
    ...
    protected function handle_export_request($db, $table_prefix, $user, $request, $logs_supported, $kb_supported, $queue_supported, array $filters)
    {
        ...
        $export = $request->variable('supporttriage_export', '', true);
        $export = trim((string) $export);
    ...
    protected function normalize_export_action($value)
    {
        $allowed = [
            ...
            'kb_created',
            'kb_synced',
            'status_auto_waiting_reply',
            'status_auto_in_progress',
            'status_auto_no_reply',
            'priority_auto_stale',
            'priority_auto_forum',
            'priority_auto_issue_type',
        ];
    ...
    protected function snippets_table($table_prefix)
    {
        return $table_prefix . 'supporttriage_snippets';
    }

    protected function logs_table($table_prefix)
    {
        return $table_prefix . 'supporttriage_logs';
    }

    protected function kb_links_table($table_prefix)
    {
        return $table_prefix . 'supporttriage_kb_links';
    }

    protected function status_table($table_prefix)
    {
        return $table_prefix . 'supporttriage_topics';
    }
    ...
    protected function get_metric_periods($user, $selected_period = 'all')
    {
        $periods = [
            [
                'key' => '24h',
                'label' => $user->lang('ACP_SUPPORTTRIAGE_METRICS_PERIOD_24H'),
                'seconds' => 86400,
            ],
            [
                'key' => '7d',
                'label' => $user->lang('ACP_SUPPORTTRIAGE_METRICS_PERIOD_7D'),
                'seconds' => 7 * 86400,
            ],
            [
                'key' => '30d',
                'label' => $user->lang('ACP_SUPPORTTRIAGE_METRICS_PERIOD_30D'),
                'seconds' => 30 * 86400,
            ],
            [
                'key' => '90d',
                'label' => $user->lang('ACP_SUPPORTTRIAGE_METRICS_PERIOD_90D'),
                'seconds' => 90 * 86400,
            ],
        ];
    ...
    protected function build_metrics_export_rows($db, $table_prefix, $user, $logs_supported, $kb_supported, $selected_period = 'all')
    {
        ...
        foreach ($this->get_metric_periods($user, $selected_period) as $period)
        {
            $since = time() - (int) $period['seconds'];
            $rows[] = [
                $period['label'],
    ...
    {
        $sql = 'SELECT COUNT(source_topic_id) AS total
            FROM ' . $this->kb_links_table($table_prefix) . '
            WHERE created_time >= ' . (int) $since;
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $db->sql_freeresult($result);
    ...
    protected function average_first_reply_since($db, $table_prefix, $since)
    {
        $sql = 'SELECT AVG(fr.first_reply_time - t.topic_time) AS avg_seconds
            FROM ' . TOPICS_TABLE . ' t
            INNER JOIN ' . $this->status_table($table_prefix) . ' st
                ON st.topic_id = t.topic_id
            INNER JOIN (
                SELECT p.topic_id, MIN(p.post_time) AS first_reply_time
                FROM ' . POSTS_TABLE . ' p
                INNER JOIN ' . TOPICS_TABLE . ' t2
                    ON t2.topic_id = p.topic_id
                WHERE p.post_visibility = 1
                    AND p.poster_id <> t2.topic_poster
                GROUP BY p.topic_id
            ) fr
                ON fr.topic_id = t.topic_id
            WHERE t.topic_moved_id = 0
                AND t.topic_visibility = 1
                AND t.topic_time >= ' . (int) $since;
        $result = $db->sql_query($sql);
    ...
    protected function average_resolution_since($db, $table_prefix, $since)
    {
        $sql = 'SELECT AVG(sol.first_solved_time - t.topic_time) AS avg_seconds
            ...
            INNER JOIN (
                ...
                FROM ' . $this->logs_table($table_prefix) . ' l
                WHERE l.new_value = ' . "'" . $db->sql_escape('solved') . "'" . '
                GROUP BY l.topic_id
            ) sol
                ON sol.topic_id = t.topic_id
            WHERE t.topic_moved_id = 0
                AND t.topic_visibility = 1
                AND sol.first_solved_time >= ' . (int) $since;
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $db->sql_freeresult($result);
    ...
    protected function get_slowest_topics($db, $table_prefix, $user, $limit = 10)
    {
        ...
        $sql = 'SELECT st.topic_id, st.forum_id, st.status_key, st.status_updated, t.topic_title, t.topic_time
            ...
            WHERE t.topic_moved_id = 0
                AND t.topic_visibility = 1
                AND st.status_key <> ' . "'" . $db->sql_escape('solved') . "'" . '
    ...
    protected function format_duration_compact($user, $seconds)
    {
        ...
        if ($seconds <= 0)
        {
            return $user->lang('ACP_SUPPORTTRIAGE_METRICS_NOT_AVAILABLE');
        }
    ...
    protected function get_logs_export_rows($db, $table_prefix, $user, array $filters)
    {
        ...
        {
            $where[] = 'l.log_time >= ' . (int) $filters['since'];
        }

        if (!empty($filters['forum_ids']))
        {
            $where[] = $db->sql_in_set('l.forum_id', array_map('intval', $filters['forum_ids']));
        }
        ...
        while ($row = $db->sql_fetchrow($result))
        {
            $rows[] = [
                ...
                (int) $row['topic_id'],
                (int) $row['forum_id'],
                $this->log_action_label($user, isset($row['action_key']) ? $row['action_key'] : ''),
                $this->log_details_plain($row, $user),
                isset($row['old_value']) ? (string) $row['old_value'] : '',
                isset($row['new_value']) ? (string) $row['new_value'] : '',
                (int) $row['related_topic_id'],
                (int) $row['related_forum_id'],
                (int) $row['user_id'],
                !empty($row['username']) ? (string) $row['username'] : $user->lang('SUPPORTTRIAGE_SYSTEM_USER'),
    ...
    protected function get_queue_export_rows($db, $table_prefix, $user, array $filters, $config)
    {
        ...
        $queue_stale_days = max(0, (int) $config['mundophpbb_supporttriage_queue_stale_days']);
        ...
        $where = [
            ...
            'st.status_key <> ' . "'" . $db->sql_escape('solved') . "'",
        ];

        if (!empty($filters['forum_ids']))
        {
            $where[] = $db->sql_in_set('st.forum_id', array_map('intval', $filters['forum_ids']));
        }

        if (!empty($filters['status']) && $filters['status'] !== 'all')
        {
            $where[] = 'st.status_key = ' . "'" . $db->sql_escape($filters['status']) . "'";
        }
        ...
        if (!empty($filters['since']))
        {
            $where[] = 'st.status_updated >= ' . (int) $filters['since'];
        }

        if (!empty($filters['stalled_only']) && $stale_threshold > 0)
        {
            $where[] = 'st.status_updated <= ' . (int) $stale_threshold;
        }
        ...
        while ($row = $db->sql_fetchrow($result))
        {
            ...
            $rows[] = [
                ...
                $this->status_label($user, isset($row['status_key']) ? $row['status_key'] : ''),
                $this->priority_label($user, isset($row['priority_key']) ? $row['priority_key'] : 'normal'),
        ...
        switch ($action_key)
        {
            ...
            case 'status_auto_no_reply':
                return $user->lang(
                    ...
                    $this->status_label($user, $old_value),
                    $this->status_label($user, $new_value)
                );

            case 'priority_change':
                return $user->lang(
                    'SUPPORTTRIAGE_LOG_DETAILS_PRIORITY',
                    $this->priority_label($user, $old_value),
                    $this->priority_label($user, $new_value)
                );
    ...
    protected function ensure_default_snippets($db, $table_prefix, $user)
    {
        ...
        foreach ($defaults as $index => $snippet)
        {
            ...
            $sql = 'INSERT INTO ' . $this->snippets_table($table_prefix) . ' ' . $db->sql_build_array('INSERT', $sql_ary);
            $db->sql_query($sql);
        }
    }
    ...
    protected function default_snippets($user)
    {
        return [
            [
                'snippet_title' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TITLE_CACHE'),
                'snippet_text' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TEXT_CACHE'),
            ],
            [
                'snippet_title' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TITLE_PROSILVER'),
                'snippet_text' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TEXT_PROSILVER'),
            ],
            [
                'snippet_title' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TITLE_DISABLE_EXT'),
                'snippet_text' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TEXT_DISABLE_EXT'),
            ],
            [
                'snippet_title' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TITLE_DEBUG'),
                'snippet_text' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TEXT_DEBUG'),
            ],
            [
                'snippet_title' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TITLE_STEPS'),
                'snippet_text' => $user->lang('SUPPORTTRIAGE_SNIPPET_DEFAULT_TEXT_STEPS'),
            ],
        ];
    }
    ...
    protected function save_snippets($db, $table_prefix, $user, $request)
    {
        ...
        if (isset($sorts[0]) && (int) $sorts[0] === 0)
        {
            unset($sorts[0]);
        }
        if (isset($actives[0]) && (int) $actives[0] === 0)
        {
            unset($actives[0]);
        }
        if (isset($deletes[0]) && (int) $deletes[0] === 0)
        {
            unset($deletes[0]);
        }
        ...
        foreach ($titles as $index => $title)
        {
            ...
            $text = $this->sanitize_text(isset($texts[$index]) ? $texts[$index] : '');
            ...
            if ($title === '')
            {
                $title = $user->lang('SUPPORTTRIAGE_SNIPPET_UNTITLED');
            }
        ...
        if ($new_title !== '' || $new_text !== '')
        {
            if ($new_title === '')
            {
                $new_title = $user->lang('SUPPORTTRIAGE_SNIPPET_UNTITLED');
            }

            $rows[] = [
                'snippet_title' => $new_title,
                'snippet_text' => $new_text,
                'sort_order' => $new_sort,
    ...
    protected function status_label($user, $status_key)
    {
        ...
        if (!in_array($status_key, $allowed, true))
        {
            return $user->lang('SUPPORTTRIAGE_STATUS_NONE');
        }
    ...
    protected function priority_label($user, $priority_key)
    {
        ...
        if (!in_array($priority_key, $allowed, true))
        {
            $priority_key = 'normal';
        }
